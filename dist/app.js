...
function createTileSet(level) {
	...
	return Array.from({ length: numTiles }).map((_, index) => {
		...
		return {
			type: mineIndexes.includes(index)
				? 1 /* TILE_TYPE.MINE */
				: 0 /* TILE_TYPE.SAFE */,
			adjacentTileIndexes,
			mineCount,
...
document.addEventListener('DOMContentLoaded', function () {
	...
	const handleFlagTile = (dataIndex) => {
		...
		updateGameState({
			tiles: [
				...
				Object.assign(Object.assign({}, tiles[dataIndex]), {
					isFlagged: !tiles[dataIndex].isFlagged,
				}),
	...
	const handleExposeTile = (dataIndex) => {
		...
		indexesToExpose.forEach((index) => {
			tiles[index] = Object.assign(Object.assign({}, tiles[index]), {
				isExposed: true,
			})
		})
